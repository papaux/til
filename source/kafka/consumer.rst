...
- ``consumer.id``:  referenced as ``group.instance.id``, is an optional unique identifier of the consumer instance. If set, only one instance with the ID is allowed per consumer group, which is used to reduce consumer rebalancing (KIP-345). If not set, the consumer will join as a dynamic member.
